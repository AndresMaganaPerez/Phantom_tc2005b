...
module.exports = class Empleados{
    ...
    static fetchEmpleadoYRol() {
        return db.execute('SELECT empleado.idEmpleado, email, empleado.nombre, apellidoPaterno, apellidoMaterno, fechaNac, fechaIngr, numTelefonico, cantidadNatgasBlocks, antiguedad, vacacionesTotales, numVacacionesLey, numVacacionesPremio, roles.idRol, nombreRolEmpresa, area.idArea, area.nombre AS "nombreArea", plaza.idPlaza, nombrePlaza FROM empleado, empleado_rol, roles, area_empleado, area, plaza, plaza_empleado WHERE empleado.idEmpleado = empleado_rol.idEmpleado AND empleado_rol.idRol = roles.idRol AND empleado.idEmpleado = area_empleado.idEmpleado AND area_empleado.idArea = area.idArea AND empleado.idEmpleado = plaza_empleado.idEmpleado AND plaza.idPlaza = plaza_empleado.idPlaza AND estatusEmpleado = 1 AND NOT area.nombre = "Recursos Humanos";')
    }
    
    static fetchEmpleadoYRolADMIN(idAdmin){
        return db.execute('SELECT e.idEmpleado, e.email, e.nombre, e.apellidoPaterno, e.apellidoMaterno, e.fechaNac, e.fechaIngr, e.numTelefonico, e.cantidadNatgasBlocks, e.antiguedad, e.vacacionesTotales, e.numVacacionesLey, e.numVacacionesPremio, roles.idRol, nombreRolEmpresa, area.idArea, area.nombre AS "nombreArea", plaza.idPlaza, nombrePlaza,dirige.idLider,l.nombre AS "NombreLider", l.apellidoPaterno AS "ApellidoPLider",l.apellidoMaterno AS "ApellidoMLider" FROM empleado e, empleado l, empleado_rol, roles, area_empleado, area, plaza, plaza_empleado, dirige WHERE e.idEmpleado = empleado_rol.idEmpleado AND empleado_rol.idRol = roles.idRol AND e.idEmpleado = area_empleado.idEmpleado AND area_empleado.idArea = area.idArea AND e.idEmpleado = plaza_empleado.idEmpleado AND plaza.idPlaza = plaza_empleado.idPlaza AND e.idEmpleado = dirige.idOperador AND e.estatusEmpleado = 1 AND l.idEmpleado = dirige.idLider AND NOT e.idEmpleado =?',[idAdmin])
    }
    ...
    static fetchEmpleadoM(nomina){
        return db.execute('SELECT empleado.idEmpleado, email, empleado.nombre, apellidoPaterno, apellidoMaterno, fechaNac, fechaIngr, numTelefonico, cantidadNatgasBlocks, antiguedad, vacacionesTotales, numVacacionesLey, numVacacionesPremio, roles.idRol, nombreRolEmpresa, area.idArea, area.nombre AS "nombreArea", plaza.idPlaza, nombrePlaza FROM empleado, empleado_rol, roles, area_empleado, area, plaza, plaza_empleado WHERE empleado.idEmpleado = empleado_rol.idEmpleado AND empleado_rol.idRol = roles.idRol AND empleado.idEmpleado = area_empleado.idEmpleado AND area_empleado.idArea = area.idArea AND empleado.idEmpleado = plaza_empleado.idEmpleado AND plaza.idPlaza = plaza_empleado.idPlaza AND estatusEmpleado = 1 AND NOT area.nombre = "Recursos Humanos" AND empleado.idEmpleado =?',[nomina])
    }
